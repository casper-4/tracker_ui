...
import {
  ...
  ShoppingCart, BarChart3, Check, RefreshCw, ChevronRight,
} from "lucide-react";
import {
  MOCK_DIET_WEEK,
  MOCK_FOOD_DB,
...
const CAT_ORDER: FoodCategory[] = ["protein", "carbs", "veggies", "dairy", "fats", "other"];
  ...
  return (
    <div className="relative flex items-center justify-center">
      ...
      <div className="absolute flex flex-col items-center">
        ...
        <span className="text-[10px] uppercase tracking-widest text-[#555]">kcal</span>
        <span className={cn("text-[10px] font-mono mt-0.5", over ? "text-[#ef4444]" : "text-[#888]")}>
    ...
    <div className="space-y-1">
      <div className="flex justify-between items-baseline">
        ...
        <span className="text-[11px] font-mono text-[#888]">
          <span style={{ color }} className="text-[#e0e0e0]">{Math.round(value)}</span>
...
function MealSlotCard({
  slot, time, ingredients, lang: language,
}: {
  slot: MealSlotId;
  time: string;
  ...
  const slotLabel = t(language as "pl" | "en", `diet_slot_${slot}` as Parameters<typeof t>[1]);
  ...
  return (
    <div className="border border-[#1f1f1f] bg-[#0a0a0a] rounded-sm overflow-hidden hover:border-[#2a2a2a] transition-colors">
      ...
      >
        <span style={{ color: slotColor }} className="shrink-0">{SLOT_ICONS[slot]}</span>
        <span className="text-[10px] uppercase tracking-[0.15em] font-mono" style={{ color: slotColor }}>
          {slotLabel}
        </span>
        ...
        <ChevronRight
          size={12}
          className={cn("text-[#444] transition-transform shrink-0", open && "rotate-90")}
        />
      </button>
          ...
          >
                ...
                ingredients.map((ing, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[10px] font-mono text-[#555]">{ing.grams}g</span>
            ...
            <div className="flex divide-x divide-[#1a1a1a] border-t border-[#1a1a1a]">
              {(
                ...
                <div key={label} className="flex-1 flex items-center justify-center gap-1 py-2">
...
export default function DietPage() {
  ...
  const [shoppingItems, setShoppingItems] = useState<ShoppingItem[]>(MOCK_SHOPPING_LIST);
  ...
  function checkAll()   { setShoppingItems((prev) => prev.map((i) => ({ ...i, checked: true }))); }
  function uncheckAll() { setShoppingItems((prev) => prev.map((i) => ({ ...i, checked: false }))); }
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-3 flex-wrap justify-end">
          <div className="flex items-center gap-1.5 border border-[#1f1f1f] px-3 py-1.5 rounded-sm bg-[#0a0a0a]">
            ...
            <span className="text-[10px] text-[#444] font-mono">/ {goals.kcal}</span>
          </div>
          {(
            [
              { key: "diet_protein", val: dayTotals.protein, target: goals.protein, color: "#ec4899" },
              { key: "diet_carbs",   val: dayTotals.carbs,   target: goals.carbs,   color: "#f59e0b" },
              { key: "diet_fat",     val: dayTotals.fat,     target: goals.fat,     color: "#a855f7" },
            ] as { key: Parameters<typeof t>[1]; val: number; target: number; color: string }[]
          ).map(({ key, val, target, color }) => (
            <div
              key={key}
              className="flex items-center gap-1.5 border border-[#1f1f1f] px-3 py-1.5 rounded-sm bg-[#0a0a0a]"
            >
              <span className="text-[10px] uppercase tracking-widest font-mono" style={{ color: color + "99" }}>
                {t(lang, key)}
              </span>
              <span className="text-sm font-bold font-mono tabular-nums" style={{ color }}>
                {Math.round(val)}
              </span>
              <span className="text-[10px] text-[#444] font-mono">/ {target}g</span>
            </div>
          ))}
        </div>
      </div>
            ...
            >
              ...
              <span
                className={cn(
                  ...
                  isActive ? "text-[#facc15]" : isToday ? "text-[#e0e0e0]" : "text-[#444]"
                )}
              >
                {dayNum(date)}
              </span>
        ...
        <div className="col-span-12 lg:col-span-5 space-y-3">
          ...
          <div className="flex border border-[#1f1f1f] bg-[#0a0a0a] rounded-sm p-0.5">
            {(["macros", "shopping"] as const).map((tab) => (
              ...
              >
                {tab === "macros" ? <BarChart3 size={11} /> : <ShoppingCart size={11} />}
                {tab === "macros" ? t(lang, "diet_macros") : t(lang, "diet_shopping_list")}
            ...
            {rightTab === "macros" ? (
              ...
              >
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <div className="flex gap-4 text-[10px] font-mono">
                    ...
                    <div className="flex flex-col items-center gap-0.5">
                      ...
                      <span className="text-[#facc15]">{Math.round(dayTotals.kcal)}</span>
                    </div>
                    <div className="flex flex-col items-center gap-0.5">
                      <span className="text-[#444] uppercase tracking-widest text-[9px]">
                ...
                <div className="grid grid-cols-3 gap-2 pt-1">
                  {(
                    [
                      { label: t(lang, "diet_protein"), pct: (dayTotals.protein * 4 / dayTotals.kcal) * 100, color: "#ec4899" },
                      { label: t(lang, "diet_carbs"),   pct: (dayTotals.carbs   * 4 / dayTotals.kcal) * 100, color: "#f59e0b" },
                      { label: t(lang, "diet_fat"),     pct: (dayTotals.fat     * 9 / dayTotals.kcal) * 100, color: "#a855f7" },
                    ]
                  ).map(({ label, pct, color }) => (
                    <div
                      key={label}
              ...
              >
                ...
                <div className="flex items-center gap-2 px-4 py-3 border-b border-[#1a1a1a]">
                  ...
                  >
                    ...
                    {listGenerated ? t(lang, "diet_generated") : t(lang, "diet_generate_list")}
                  </button>
                ...
                <div className="max-h-[420px] overflow-y-auto custom-scrollbar divide-y divide-[#0f0f0f]">
                  {CAT_ORDER.filter((cat) => groupedShopping[cat]).map((cat) => (
                    <div key={cat}>
                      {/* category header */}
                      <div className="sticky top-0 flex items-center gap-2 px-4 py-2 bg-[#0a0a0a] border-b border-[#141414]">
                        ...
                        <span className="ml-auto text-[9px] font-mono text-[#333]">
                          {groupedShopping[cat]!.filter((i) => !i.checked).length}/
                        ...
                        >
                          ...
                          <span
                            className={cn(
                              ...
                              item.checked ? "text-[#444] line-through" : "text-[#ccc]"
                            )}
                          >
                            {item.name}
                          </span>
