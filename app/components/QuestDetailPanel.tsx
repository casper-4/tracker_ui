...
const STATUS_OPTIONS: {
  value: QuestStatus;
  labelKey: "status_open" | "status_planned" | "status_in_progress" | "status_completed";
}[] = [
...
export default function QuestDetailPanel({
  ...
  const [editActualDuration, setEditActualDuration] = useState(
    initialQuest.actualDuration != null ? String(Math.round(initialQuest.actualDuration / 60)) : ""
  );
  ...
  useEffect(() => {
    ...
    setEditActualDuration(initialQuest.actualDuration != null ? String(Math.round(initialQuest.actualDuration / 60)) : "");
  ...
  const handleSaveDescription = useCallback(() => {
    const updated = { ...quest, description: editDescription.trim() || quest.description };
    setQuest(updated);
    // TODO: [DATA] persistence will go here
    onQuestChange?.(updated);
    setIsEditing(false);
    ...
    >
      <div className="flex flex-col h-full" style={{ width: PANEL_WIDTH }}>
        ...
        <div className="px-5 pt-4 pb-3 border-b border-[#1f1f1f] shrink-0">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <p className="text-[10px] uppercase tracking-widest font-mono mb-1" style={{ color: skillColor }}>
                {skillName ?? "—"}
              </p>
            ...
            <div className="flex items-center gap-2 shrink-0 pt-0.5">
              <button
                ...
                style={
                  quest.isRecurring
                    ? { backgroundColor: skillColor + "22", borderColor: skillColor, color: skillColor }
                    : { backgroundColor: "transparent", borderColor: "#333", color: "#444" }
                }
          ...
          >
            <div className="px-5 py-4 space-y-5">
                ...
                {isEditing ? (
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <button type="button" onClick={handleSaveDescription} className="px-3 py-1.5 rounded bg-[#333] hover:bg-[#444] text-white text-xs">{t(lang, "quest_save")}</button>
                      <button type="button" onClick={() => { setEditDescription(quest.description); setIsEditing(false); }} className="px-3 py-1.5 rounded border border-[#333] text-[#888] hover:text-white text-xs">{t(lang, "quest_cancel")}</button>
                    </div>
                  </div>
                ) : (
                  <p className="text-sm text-[#aaa] whitespace-pre-wrap leading-relaxed">{quest.description || "—"}</p>
                )}
              </div>
              ...
              <div>
                ...
                <p className="text-sm text-[#aaa] font-mono">{formatDuration(quest.duration)}</p>
              </div>
                ...
                {isEditingActual ? (
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      autoFocus
                    />
                    <span className="text-[10px] text-[#555] uppercase tracking-widest">min</span>
                    <button type="button" onClick={handleSaveActualDuration} className="px-3 py-1.5 rounded bg-[#333] hover:bg-[#444] text-white text-xs">{t(lang, "quest_save")}</button>
                    <button type="button" onClick={() => { setEditActualDuration(quest.actualDuration != null ? String(Math.round(quest.actualDuration / 60)) : ""); setIsEditingActual(false); }} className="px-2 py-1.5 rounded border border-[#333] text-[#888] hover:text-white text-xs">{t(lang, "quest_cancel")}</button>
                  </div>
                ) : (
                  <p className="text-sm font-mono" style={{ color: quest.actualDuration != null ? MOCK_STATUS_COLORS.completed : "#3a3a3a" }}>
                    {quest.actualDuration != null ? formatDuration(quest.actualDuration) : "—"}
                  </p>
                )}
              </div>
              )}
                      ...
                      <>
                        <ul className="space-y-1.5">
                          {visible.map((entry, i) => {
                            ...
                            const labelKey = `status_${entry.status}` as "status_open" | "status_planned" | "status_in_progress" | "status_completed";
                            return (
                              <li key={i} className="flex items-center gap-2">
                                <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: color }} />
                                <span className="text-[10px] uppercase tracking-wider w-20 shrink-0" style={{ color }}>
                                  {t(lang, labelKey)}
                                </span>
                        ...
                        {hasMore && (
                          ...
                          >
                            {historyExpanded ? `▲ ukryj` : `▼ +${reversed.length - 3} więcej`}
                          </button>
                        )}
                      </>
                    );
                  })() : (
                    <p className="text-[10px] font-mono text-[#2a2a2a]">{t(lang, "quest_history_empty")}</p>
                  )}
                </div>
              </div>
        ...
        <div className="px-5 py-3 border-t border-[#151515] shrink-0">
          <p className="text-[10px] font-mono text-[#2e2e2e] truncate" title={quest.id}>
            {quest.id}
          </p>
        </div>
